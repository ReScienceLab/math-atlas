...
export function ProblemCard({
  ...
  const visualization = (
    ...
    >
      {Viz ? (
        ...
        <div className="absolute inset-0 flex items-center justify-center text-[11px] text-[var(--gray-700)]">viz</div>
      )}
    </div>
  );
  ...
  return (
    ...
    >
      ...
      {!zen && (
        <Link href={`/problems/${problem.slug}`} className="block px-3 py-3 transition-colors hover:bg-white/[0.035]">
          <div>
            <div className="mb-2 flex items-center justify-between gap-2">
              <span className={`inline-flex shrink-0 font-[var(--font-mono)] text-[8px] font-medium uppercase px-1 py-px border ${statusColor[problem.status]}`}>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-[var(--font-mono)] text-[9px] text-white/[0.72]">{problem.year}</span>
            </div>
          </div>
        </Link>
      )}
